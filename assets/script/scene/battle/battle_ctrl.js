...
cc.Class({
    ...
    initMission: function () {
        if (!this._currHurdleConfig)
            return;
        var cfg = this._currHurdleConfig;
        for (var i = cfg.mission.length - 1; i >= 0; i--)
            this._missions.push(cfg.mission[i]);
        this._missionStartTime = 0;
        this._killNum = 0;
    },
    
    changeHurdle: function (id) {
        this._startted = false;
    ...
    transform: function (type, born) {
        ...
        if (type === 0) { //变黑
            ...
            action = new cc.Sequence(new cc.FadeIn(time),
            ...
            action = new cc.Sequence(new cc.FadeOut(time),
                new cc.CallFunc(function () {
                    if (born) {
    ...
    onHurdleStepLoaded: function (bit, err, prefab) {
        switch (bit) {
    ...
    createMonster: function (id, pos, dir) {
        var self = this;
    ...
    getActorByRegion: function (actor, region) {
        ...
        for (var i = 0; i < this._monsters.length; i++) {
            ...
            if (Math.abs(actor.node.y - mon.node.y) < 30
            && (cc.Intersection.rectRect(mon.getCollision(), region)))
               mons.push(mon);
        }
        return mons;
    },
    ...
    update: function (dt) {
        ...
        if (!this._currMission && !this.launchMission())
            return;
    ...
    showResultFace: function (success) {
        
    },

    playEffect: function (id, layer, pos, flip, keep) {
        var self = this;
